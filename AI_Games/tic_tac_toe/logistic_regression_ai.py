...
class Player(object):
    """
    Provides all the methods that a Player must execute in order to play a game of Tic-Tac-Toe. Players_symbol is not
    necessary if you just want to get the next best move. This can be seen in tic_tac_toe.py where no symbol was passed
    when Player object is instantiated.
    """

    def __init__(self, player_target_function_weight_vector, player_symbol=None):
        self.playerSymbol = player_symbol  # "X" or "O"
        self.playerTargetFunctionWeightVector = player_target_function_weight_vector
    ...
    def look_for_legal_moves(self, board_state, player_symbol):
        ...
        :return: legal_moves_index: a list of indexes for next possible move.
        """

        legal_moves = []  # Keeps track of the board with new legal move.
        legal_moves_index = []  # keeps track of the index. For use in tic_tac_toe_emulator.py
        for i in range(1, 10):  # Iterating from 1 to 9
            if board_state[i] == ' ':  # If the position is empty.
                legal_moves_index.append(i)
        ...
        return legal_moves, legal_moves_index
    ...
    def chooseMove(self, board, playerSymbol1, playerSymbol2):
        ...
        legalMoveScores = [self.calculateNonFinalBoardScore(self.playerTargetFunctionWeightVector,
                                                            self.extractFeatures(board, playerSymbol1, playerSymbol2)) for i
                           in legalMoves]
...
class PerformanceSystem:
    ...
    class simulates Tic-Tac-Toe game.
    """
...
def train(numTrainingSamples=10):
    ...
    print("Final Learnt Weight Vector: \n" + str(playersTargetFunctionWeightVectors[0]))
